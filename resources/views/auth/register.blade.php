     ...
     <div class="w-full py-14 px-8 sm:p-[70px] lg:px-14 xl:px-[90px]">
      ...
      <form action="{{route('register')}}" method="post">
       ...
       <div class="flex flex-wrap">
        <div class="w-full">
         <button id="submit-button" class="bg-secondary mb-3 py-4 rounded-lg w-full cursor-pointer px-5 text-white transition hover:bg-opacity-90" disabled>
          Create Account
         </button>
        </div>
       </div>
...
termsLink.addEventListener('click', (e) => {
  ...
  termsModal.classList.remove('hidden'); // Modalı görünür yap
});

// Modal kapanır
closeModal.addEventListener('click', () => {
  termsModal.classList.add('hidden'); // Modalı gizle
});

// Checkbox değişirse buton kontrolü
checkbox.addEventListener('change', () => {
  submitButton.disabled = !checkbox.checked; // Checkbox işaretli değilse buton devre dışı
});
...
form.addEventListener('submit', (e) => {
  if (!checkbox.checked) {
    ...
    alert("Lütfen hizmet şartlarını kabul edin."); // Uyarı göster
  }
});
